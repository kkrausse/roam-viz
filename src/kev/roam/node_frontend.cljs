    ...
    (r/with-let [collapsed* (r/atom (boolean default-collapsed))]
      [:div (deep-merge {:style {:background-color "grey.200"
       ...
       [:div {:style    {:display        "inline-flex"
                         :flex-direction "row"}}
        [:a {:on-click (fn [e] (swap! collapsed* not))
...
(defn linkable-heading [& {:keys [db component id title text]}]
  (let [theme (useTheme)]
    [component (merge {:style {:margin 0}}
                      (when id
...
;; FIXME: should put more into grouping blocks
